...
def test_emissions_channel_iterates_one(testdir, emissions_channel_string):
    ...
    async def test(request):
        ...
        {emissions_channel_string}(signals=[instance.signal]) as emissions:
...
def test_open_emissions_channel_does_not_close(testdir):
    """Opening emissions channel does not close send and receive channels on exit."""
    test_file = r"""
    from qtpy import QtCore
    import qtrio
    ...
    async def test(request):
        ...
        assert not emissions.channel._closed and not emissions.send_channel._closed
    """
...
def test_enter_emissions_channel_closes(testdir):
    ...
    async def test(request):
        ...
        assert emissions.channel._closed and emissions.send_channel._closed
    """
    # TODO: any public way to check the channels are closed?
    testdir.makepyfile(test_file)
